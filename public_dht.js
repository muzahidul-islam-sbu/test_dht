...
async function main() {
    ...
    node.addEventListener('peer:connect', async (connection) => {
        ...
        let retrievedValue = node.services.aminoDHT.findPeer(connection.detail);
    ...
    // Start the DHT
    await node.services.aminoDHT.setMode('server');
    await node.services.aminoDHT.start();
    await node.start();
    ...
    rl.on('line', async (input) => {
        ...
        switch (command) {
            case 'put':
                ...
                await node.services.aminoDHT.refreshRoutingTable()
                break;
            case 'get':
                if (args.length !== 1) {
    ...
    // node.services.aminoDHT.put(a, b, {useNetwork: true, useCache: false})
    rl.on('close', () => {
        console.log('Exiting...');
        process.exit(0);
    });
...
async function putKeyValue(node, key, value) {
    ...
    let retrievedValue = node.services.aminoDHT.put(keyUint8Array, valueUint8Array ,{useNetwork: true, useCache: false});
    for await (const queryEvent of retrievedValue) {
        console.log('put' , queryEvent)
    }
    console.log(`Key "${key}" with value "${value}" successfully added to the aminoDHT.`);
}
...
async function getValue(node, key) {
    ...
    retrievedValue = node.services.aminoDHT.get(keyUint8Array,  {useNetwork: true, useCache: false});
    for await (const queryEvent of retrievedValue) {
